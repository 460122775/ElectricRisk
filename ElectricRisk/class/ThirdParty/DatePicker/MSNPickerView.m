...
//  Botianxia
//
//  Created by Mason on 16/3/18.
//  Copyright © 2016年 Mason. All rights reserved.
//
//
#define MSToobarHeight 40
#import "MSNPickerView.h"

@interface MSNPickerView ()<UIPickerViewDelegate,UIPickerViewDataSource>
...
-(void)setFrameWith:(BOOL)isHaveNavControler{
    CGFloat toolViewX = 0;
    CGFloat toolViewH = _pickeviewHeight+MSToobarHeight;
    CGFloat toolViewY ;
...
-(void)setUpPickView{
    ...
    pickView.frame=CGRectMake(0, MSToobarHeight, [UIScreen mainScreen].bounds.size.width, pickView.frame.size.height);
...
-(void)setUpDatePickerWithdatePickerMode:(UIDatePickerMode)datePickerMode{
    ...
    datePicker.frame=CGRectMake(0, MSToobarHeight, [UIScreen mainScreen].bounds.size.width, datePicker.frame.size.height);
...
-(UIToolbar *)setToolbarStyle{
    ...
    UIBarButtonItem *rightItem=[[UIBarButtonItem alloc] initWithTitle:@"确定  " style:UIBarButtonItemStyleDone target:self action:@selector(doneClick)];
    rightItem.tintColor = Color_me;
    toolbar.items=@[leftItem,centerSpace,rightItem];
    
    return toolbar;
}
-(void)setToolbarWithPickViewFrame{
    _toolbar.frame=CGRectMake(0, 0,[UIScreen mainScreen].bounds.size.width, MSToobarHeight);
}
...
-(NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component
{
    ...
    DLog(@"!------>numberOfRowsIncomponet%ld",component);
